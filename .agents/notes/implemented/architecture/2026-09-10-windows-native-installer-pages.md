...
A new installation opens with three introduction steps: the product, the recommended official DeepSeek models with the CommandCode plan, and the TianmaCoder competition. The steps replace one page’s caption, title, and body lines in place under Back and Next, so they add no NSIS page, and `--updated` runs skip them. The installation, introduction, and completion pages share one shell that owns the window frame, brand bitmap, fonts, and copyright caption.
