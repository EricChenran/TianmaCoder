...
|---|---|---|
| `includeHarnessIdentity` | `true` | Include the fixed `You are an AI agent powered by TianmaCoder Harness.` first-party opener at order −1000. Set false only when a compatibility deployment owns the complete system prompt. |
...
```markdown
You are an AI agent powered by TianmaCoder Harness.
```
