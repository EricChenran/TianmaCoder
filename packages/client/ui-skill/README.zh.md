...
`dsh-client-ui-skill` 让用户通过 `/` 建议选择或直接键入 `/name` 来调用 skill（技能），并拥有侧栏的「技能」页面，可在其中查看与开关本配置的 skill。同一条字面命令可以从 Web 编辑器、TUI 和 ACP（Agent Client Protocol）一致地加载 skill；如果名称与宿主命令相同，它仍会解析为该命令。skill 调用在对话中显示为可展开的 `Instructions` 卡片；即使已安装的 skill 目录发生变化，卡片落定后的内容仍保持稳定。
...
侧栏的「技能」入口打开一个全局页面，管理主列当前会话的 skill。两个行分组对应开关的两种状态：当前会话组成正在提供的 skill，以及宿主隐去的名称。每一行带 skill 名称、路由描述、模型不可用时的仅限用户标记、说明文件查看入口，以及开关本身。

开关写入 skill 注册表的 `disabled` 列表——即 `skill` 条目的设置节（见 `skill-registry-settings.ts`）——因此被关闭的 skill 会同时离开 `/` 菜单、模型目录与该注册表的其他所有读取，本会话与其他任何地方皆是如此，且状态在重启后保留。写入进行中时该行的开关锁定；被拒绝的写入会报告失败并保留已接受的值。页面通过共享的配置表单服务读取注册表节，因此另一个标签页里的编辑器拨动的是同一批开关。开关只写宿主文档：页面从不改动 skill 文件或提供方。
...
这些限制定义引用、工具行与「技能」页面何时回退到通用行为；它们是当前包约束。
...
- **「技能」页面列出的是会话组成，而非整个安装**：目录与 `/` 菜单使用同一个按会话寻址的读取，因此页面需要会话，没有会话时显示空选择引导。本页面从未见过启用状态的已关闭名称，在其名称之外没有其他信息，直到某次读取返回它。
