...
一条字面系统提示词节 `tianma:department`，承载所选部门的完整规则：技术部覆盖后端、前端、运维三类纪律与交付自检；商务部覆盖需求文档、报价单、案例检索与共通的商务红线。文本与 `departmentPrompt(department)` 逐字节一致，且该节不替换任何内容——它与 persona、工作区指令、行为规范并列组合。

#### Token 影响

按 4 字符/token 的启发式估计，技术部约 6,400 token、商务部约 2,700 token，作为系统提示词的一部分每请求一次、跨轮不变。用户未选择的模式不产生任何开销。
