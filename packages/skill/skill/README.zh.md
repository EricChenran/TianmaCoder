...
像任何 Cordis 插件一样加载即可。配置项限制内存中保留的已完成提供方目录数量，并决定哪些名称从所有接口隐去；其余都是提供方行为。
...
| `disabled` | `[]` | 保持安装但从所有目录与查找中隐去的 skill 名称；这是活动字段，Web 客户端的「技能」页面会实时写入 |
...
注册表没有 TTL：只有提供方调用其注册作用域内的 `invalidate()`，或发生运行时注册或释放时，才会清除已完成的目录。每次失效都会递增 revision、清空缓存，并发出不带过滤条件的 `skills/change` 事件；消费方用各自的查找选项重新获取。`invalidate()` 仅当接收它的那条精确注册仍处于活动状态时才生效，因此延迟回调无法干扰同名替代提供方。`disabled` 列表的变化以同样方式清除目录：该字段是 Loader 原地提交的活动引用，注册表在下次读取时比对它据以过滤的快照。
